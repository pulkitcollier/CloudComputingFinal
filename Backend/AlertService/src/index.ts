...
export async function handler(event: any, context: Lambda.Context, callback: Lambda.Callback): Promise<void> {
    ...
    for (var i = 0; i < lateTweet.length; i++) {
        ...
        if (acount > tres.activity_count_std) {
            await conn.query('INSERT INTO alert (userid, friendid, alertcontent, createtime) VALUES ($1, $2, $3, $4)', [lt.userid, nt.userid, `Your Friend ${nt.userid} is having abnormal activities.`, new Date().toUTCString()]);
            sns.publish({
        ...
        if (sscoreabs > tres.sentiment_score_std) {
            await conn.query('INSERT INTO alert (userid, friendid, alertcontent, createtime) VALUES ($1, $2, $3, $4)', [lt.userid, nt.userid, `Your Friend ${nt.userid} is being ${sscore > 0 ? 'happy' : 'sad'}.`, new Date().toUTCString()]);
            sns.publish({
